 ...
 * so we force its generation.
 ...
 ANDN(a,b) = b & ~a. This is used to get the superchars
 */
...
void get_conf_stride_1(const u8 *itPtr, UNUSED const u8 *start_ptr,
                       ...
                       const u64a *ft, u64a *conf0, u64a *conf8, m128 *state) {
    /* +1: the zones ensure that we can read the byte at z->end */
    assert(itPtr >= start_ptr && itPtr + ITER_BYTES <= end_ptr);

    // compute superchars itPtr & fdr->domainMask
    ...
    st4 = or128(st4, st6);
    st0 = or128(st0, st4);
    *state = or128(*state, st0);

    *conf0 = movq(*state);
    *state = rshiftbyte_m128(*state, 8);
    *conf0 ^= ~0ULL;
...
/* FDR_MAIN_LOOP inlined at its single call site below. */
static never_inline
    ...
    for (size_t curZone = 0; curZone < numZone; curZone++) {
        ...
        do {                                                                 
            ...
            for (const u8 *itPtr = start_ptr; itPtr + ITER_BYTES <= end_ptr; 
                ...
                u64a conf0;                                                   
                u64a conf8;                                                   
                get_conf_stride_1(itPtr, start_ptr, end_ptr,                 
                                    domain_mask_flipped, ft, &conf0, &conf8, 
                                    &state);                                 
...
hwlm_error_t fdrExec(const struct FDR *fdr, const u8 *buf, size_t len,
    ...
    const struct FDR_Runtime_Args a = {
        ...
        cb,
        scratch,
        buf + len, /* flood detection disabled: set to end-of-buffer */
        0
    };
    if (unlikely(a.start_offset >= a.len)) {
        return HWLM_SUCCESS;
    } else {
        return fdr_engine_exec(fdr, &a, groups);
    }
}